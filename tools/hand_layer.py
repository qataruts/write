...
def unfold(path: list, marks: list, pen: float) -> None:
    ...
    **وجهةُ الفتح يدلّ عليها ذراعا الطيّة**: الضلعُ الصاعد يميل إلى حيث جاء القلم
    ...
    # 🔴 **والعرضُ المتحرك سيّدُ الحكم في مادّة النسخ** (حكمُ المالك ٢٥ أغسطس ٢٠٢٦:
    # «المهمُّ أن يكون شكلُ الكتابة التي يتدرّب عليها الطفلُ صحيحاً وجميلاً… ويتعلّم
    # بالأنميشن… والمحرّكُ وظيفتُه التقييم ولا يوقف شيئاً»): **فتحُ الذراعين يجعل
    # السنَّ حلقةً ضيّقةً في العرض** — رآها المالكُ فقال «الأنميشن لا تمشي بشكل
    # صحيح»، ورأيتُها بعيني في «شَمْسْ». ⇐ **الذراعان ينطبقان** فيخرج السنُّ سنّاً،
    # **وثمنُه مقيسٌ ومُعلَن**: قياسُ الطريقة (الماشي) يضعف في الكلمات، **وقياسُ
    # الشكل — وهو مصدرُ النجوم — لا يمسّه شيء** لأنّه لا يقرأ الطيّة أصلاً.
    # (والانطباقُ التامّ يكسر حارسَ «مفكوكان لا منطبقان» — وأدناه ثُمنُ القلم:
    # فرجةٌ لا تُرى في الكلمة وتكفي ألّا ينطبق الضلعان.)
    grip = pen / 8.0
