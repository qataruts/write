...
//
// ١) **أولُ إملاء كلمةٍ يقع قبل أوّل بستان إملاء**: فلا يبلغ الطفلُ محطةً اسمُها
//    «إملاء» ولمّا يُملِ كلمةً قطّ (وهو وجهُ أ١ القياسيّ: المحطةُ تقيس غيرَ ما تعلن).
...
ok(firstSayNode >= 0 && firstDictation && firstDictation.index < firstSayNode,
  `ووقع **قبل** أوّل بستان إملاء (العقدة ${arNum(firstDictation?.index ?? -1)} `
  + `قبل ${arNum(firstSayNode)}) — فلا محطةَ إملاءٍ يدخلها ولمّا يُملِ`);
