...
console.log('\n— ٢ه) سياسةُ الإطلاق في آلة الخطوة الحرّة: قبولٌ · انتظارٌ · ردّ —');
{
  ...
  // ٣) **حدُّ القواعد الهندسية — يُقال باسمه لا يُموَّه** (عيادةُ ٢٢ أغسطس):
  {
    ...
    // (د) 🔴 **وأولُ شاهدٍ من إصبعٍ حقيقية** (حصادُ ٢٣ أغسطس، الصفُّ ١٤ — أثمنُ
    // ما فيه بقول المالك): غينٌ وسطيةٌ بيسرى طفلةٍ في السادسة، عينُ الأب تردّها
    // **والحَكَمُ لا يجد فيها مأخذاً واحداً** (تغطية ٩٢٪ ودقة ١٠٠٪ ونقطتُها في
    // مقعدها) — فالعمى ليس عن خربشة هجومٍ فحسب بل عن شكلٍ فقدت بنيتُه وبقي
    // إشغالُه. يغلقه بابُ البنية — وانقلابُه أحمرَ إغلاقُ الدَّين: حدِّث الحارس.
    const ghain = JSON.parse(readFileSync(new URL('fixtures/field-ghain-medial-eye-reject.json', import.meta.url), 'utf8'));
    ok(pen.judgeShape(PATHS['غ'].medial, ghain.strokes).ok === true,
      '🔴 دَينُ البنية (٤): غينُ الميدان المردودةُ بعين الأب تُقبَل — يغلقه بابُ البنية');
  }
