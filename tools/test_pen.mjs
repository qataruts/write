...
const modules = ['audio.js', 'curriculum.js', 'gate.js', 'main.js', 'parent.js', 'pen.js',
  'pendev.js', 'progress.js', 'review.js', 'ui.js'];
...
// **ولكلِّ حالةٍ مسارُها المرجعيّ من الملفّ نفسِه**: `sample` هو شكلُ صفحة التجربة
// بعينه (يكتبه المولّد من `pendev.js`)، و`ring` شكلٌ مغلق للعدّة وحدها — فالعدّةُ
// شاهدٌ تامّ: مسارُها معها لا يفترقان، ويحرس المولّدُ تطابقَ الأول مع الوحدة.
const refOf = (item) => traces.refs[item.ref];
ok(JSON.stringify(traces.refs.sample) === JSON.stringify(dev.SAMPLE),
  'والمسارُ المرجعيّ في العدّة عينُ ما تعرضه صفحةُ التجربة — لا شكلان يفترقان');
...
// ————— مطالبةٌ مؤجَّلة تُطلق نفسها (نمطُ الجلسة ٠ المُقَرّ) —————
// يومَ تُؤلَّف مساراتُ الحروف في الجلسة ٢ يمتلئ `PATHS`، فيصير عرضُ شكلٍ هندسيّ
// بدلَ حرفٍ موجودٍ على القرص كذباً — فيحمرّ هذا السطرُ ويطالب بتحويل الصفحة.
ok(!dev.realPathsExist() || devCode.includes('pathOf') || devCode.includes('PATHS['),
  dev.realPathsExist()
    ? 'دخلت المنهجَ مساراتُ حروفٍ حقيقية — فلْتتحوّل صفحةُ التجربة إليها ولْيسقط الشكلُ الهندسيّ'
    : 'ولا مسارَ حرفٍ في المنهج بعدُ (الجلسة ٢) — فالشكلُ الهندسيّ في موضعه، ويسقط يومَ تصل الحروف');
