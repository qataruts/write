...
console.log('\n— ١) خارج تطبيق الطفل: صفحةٌ مستقلّة، ولا تدخل رحلةَ الطفل —');
{
  ...
  // **الذكرُ في sw.js صار استثناءً لا دخولاً** (إصلاحُ ٢٠ أغسطس: قائمةُ OUTSIDE تمرّر
  // الجاراتِ فلا يبتلعها ردُّ التنقّل) — فيُفحَص العهدُ في موضعيه لا بذكر الاسم:
  // القشرةُ (SHELL) لا تحملها، وقائمةُ الاستثناء تحملها صراحةً.
  const shellBlock = (sw.match(/const SHELL = \[[^\]]*\]/) || [''])[0];
  ok(shellBlock.length > 0 && !/arena/.test(shellBlock),
    'ولا تدخل قشرةَ العمل دون إنترنت (قائمةُ SHELL) — فليست من التطبيق');
  ok(/const OUTSIDE = \[[^\]]*'arena\/'/.test(sw),
    'وعاملُ الخدمة يستثنيها صراحةً (OUTSIDE) — فلا يبتلعها ردُّ التنقّل');
