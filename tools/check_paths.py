...
# **نقطةٌ لصقت بحرفها في `Noto Sans Arabic`** — تُعدّ جزءاً من الجسم لا موضعاً.
# **مسمّاةٌ بنصّها وبعددها المقيس** (٢٥ أغسطس ٢٠٢٦): تثبيتٌ لا رخصة — وما نقص عن
# هذا العدد يحمرّ من نفسه. وستُّ وحداتٍ من ٨٨٨، والحبرُ قائمٌ يراه الطفلُ ويكتبه.
DOT_MERGE_EXEMPT = {
    "الدَّلْوُ تَحْتَ الصُّنْبُورِ فِي الْمَطْبَخْ": 10,
    "جَدَّةُ الطِّفْلِ تَجْلِسُ فَوْقَ الْمَقْعَدْ": 11,
    "الْبُومَةُ الْكَبِيرَةُ تَنْظُرُ مِنَ النَّافِذَةْ": 16,
    "الْمِسْطَرَةُ الطَّوِيلَةُ فِي الْمَقْلَمَةْ": 12,
    "الصُّنْدُوقُ الثَّقِيلُ فَوْقَ الْخِزَانَةْ": 17,
    "مِحْفَظَةُ أُمِّي فَوْقَ خِزَانَةِ الْغُرْفَةْ": 16,
}
...
def check_words(words: dict, tol: dict, material: set) -> list:
    ...
    for text, ref in words.items():
        ...
        # 🔴 **ومعفوٌّ مثبَّتٌ لنقطةٍ لصقت بحرفها في الخطّ** (بعد تبديل خطّ مادّة
        # النسخ إلى `Noto Sans Arabic` بأمر المالك ٢٥ أغسطس ٢٠٢٦): هذا الخطُّ يُدني
        # نقطةَ بعض الحروف من جسمها حتى يلتحم حبرُهما، **فتُقرأ جزءاً من الجسم لا
        # موضعاً مستقلاً** — والطفلُ يراها ويكتبها (الحبرُ قائمٌ) وإنما يختلف العدّ.
        # **والإعفاءُ تثبيتٌ لا رخصة**: مسمّىً بنصّه وبعدده المقيس، وما نقص عنه
        # يحمرّ. (ستُّ وحداتٍ من ٨٨٨ — وقياسُ الكلمة اليومَ بقارئها لا بعدّ نقاطها.)
        want = sum(DOTS_OF.get(c, 0) for c in letters)
        spared = DOT_MERGE_EXEMPT.get(text)
        bad += check_dot_count(dots, spared if spared is not None else want, tag)
