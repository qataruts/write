...
function build() {
  ...
  for (const text of WORD_CASES) {
    ...
    /* 🔴 **وقياسُ الطريقة مرفوعٌ عن الكلمات بحكم المالك** (٢٥ أغسطس ٢٠٢٦: «في
       الكلمات أقصد نلغيه»): مادّةُ النسخ تعود على أثرها كثيراً (القطعُ إلى أدناه
       بأمره) فيقرأ الماشي رجوعَها تحت رجفة الطفل انحرافاً. **فحالُ الرجفة على
       الكلمة تُقاس وتُطبع دَيناً مسمّى ولا يُحكَم بها** — وقياسُ الشكل (مصدرُ
       النجوم) على حاله يُحرَس فوقها. والحرفُ لا يمسّه هذا: بوابتُه خضراء. */
    add(`${id}-drift`, { shape: true, exact: id !== 'word-3',
      debt: 'method-lifted-on-words', tolerance: ref.tolerance },
