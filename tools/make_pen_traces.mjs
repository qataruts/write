...
import { SAMPLE } from '../app/js/pendev.js';
...
function build() {
  ...
  return {
    ...
    refs_note: '`sample` هو شكلُ صفحة التجربة بعينه (app/js/pendev.js) — يُكتب هنا من '
      + 'الوحدة نفسِها، فتغييرُه هناك يُسقِط الفحصَ الذاتي؛ و`ring` شكلٌ مغلق للعدّة وحدها.',
...
if (process.argv.includes('--self-test')) {
  ...
  // **والمسارُ المرجعيّ محبوسٌ مع مساراته**: `refs.sample` يُكتب من `pendev.js` نفسِه،
  // فتغييرُ شكل صفحة التجربة يُسقِط هذا الفحصَ حتى تُعاد المساراتُ عليه — ولا تبقى
  // عدّةٌ تحكم على شكلٍ غير الذي تراه الشاشة.
  ok(JSON.stringify(saved.refs) === JSON.stringify(made.refs),
    `والمساراتُ المرجعية في الملفّ عينُ مصادرها (${Object.keys(made.refs).join('، ')})`);
