...
/**
 * **الحركةُ تُؤجَّل والحرفُ يُدرَّب** — حكمُ المالك (١٩ أغسطس ٢٠٢٦): «لا تُعرَض ولا
 * تُطلَب»، و«نؤجّل الشدّةَ والتنوين». **وهي قاعدةٌ تولّد الحكم لا قائمةٌ تُعَدّ**:
 * ما زال بتجريد النصّ فحركةٌ، وما بقي فحرفٌ — فالهمزةُ والتاءُ المربوطةُ والألفُ
 * المقصورة **حروفٌ تُدرَّب** (وتركُها خطأٌ إملائيّ)، والفتحةُ والسكونُ والشدّةُ
 * والتنوينُ **علاماتٌ تُؤجَّل**.
 */
const HARAKA = /[\u064B-\u0652\u0670]/;
const marksOf = (text) => [...text]
  .filter((ch) => ch !== ' ' && !isLetter(ch) && !HARAKA.test(ch));
...
const skillSigns = rc.SKILLS
  ...
  // **ومهارةٌ كلُّ علاماتها حركاتٌ لا محطّةَ لها هنا** (حكمُ المالك): الشدّةُ
  // والتنوينُ يخرجان بالقاعدة نفسِها، **ولامُ التعريف تبقى** — حرفان لا حركة.
  .filter((s) => !s.marks.every((m) => [...m].some((c) => HARAKA.test(c))))
...
// **والشدّةُ والتنوينُ خرجا من العهد بحكم المالك** (١٩ أغسطس ٢٠٢٦) — يُؤجَّلان مع
// الحركات، **ولا يُطلَبان ولا يُعرَضان**. ويبقى العهدُ على المرسوم بهمزةٍ: حروفٌ.
const Q3 = [...Object.keys(VARIANTS).filter((ch) => VARIANTS[ch].mark.includes('همزة'))];
