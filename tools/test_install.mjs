...
// **وصفحةُ التعريف لا تُحمّلها** — وقد حان موعدُ الفرق الذي كان هذا السطرُ يترقّبه
// («وذكرُ الشريط نصّاً للجلسة ١١»): الجلسةُ ١١ **تصف** الشريطَ للمعلّم وتسمّي وحدتَه
// في تعليق، **ولا تشغّله**. فالمقيسُ صار **التحميلَ لا الذِّكر**: لا وسمَ نصٍّ ولا
// موردٌ يُجلَب باسمها. وصفرُ الجافاسكربت في الصفحات الأربع كلِّها يفرضه
// `test_welcome.mjs` أشدَّ من هنا (لا `<script>` ولا `on*=` ولا عاملَ خدمة).
const welcome = readFileSync(new URL('../app/welcome/index.html', import.meta.url), 'utf8');
const loads = [...welcome.matchAll(/(?:src|href)=["']([^"']*install\.js[^"']*)["']/g)];
ok(loads.length === 0 && !/<script/i.test(welcome),
  'وصفحةُ التعريف تصف الشريطَ ولا تُحمّله — صفرُ جافاسكربت فيها عهدٌ قائم'
  + (loads.length ? ` (تجلب: ${loads.map((m) => m[1]).join('، ')})` : ''));
