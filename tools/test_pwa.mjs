...
//   ٥) **قفلُ المقياس بأجزائه الثلاثة** (منقولُ اقرأ الميداني، الجلسة ٤ب): لا زومَ على
//      سطح الكتابة، ولوحُ القلم يبقى على إعلانه، والتعريفُ يبقى قابلاً للتكبير. في §٦.
//
...
// ————— ٦. قفلُ المقياس — لا زومَ على سطح الكتابة (منقولٌ من اقرأ، `read@9220ab1`) —————
//
// **بلاغُ طفلةٍ حقيقية**: iPadOS يسترجع التطبيقَ المثبَّت من الخلفية **مكبَّراً**
// أحياناً (عيبُ منصّةٍ معروف)، والنقرُ المزدوج يكبّر الشاشة. **وهو عندنا أخطر منه
// في اقرأ**: أصابعُ الطفل وقلمُه على اللوح نقراتٌ متتابعة، وزومٌ عارضٌ أثناء رسم حرفٍ
// يفسد التمرين — والمقياسُ المنحرف يزيح ما تحت إصبعه عمّا تحت قلمه.
//
// والقفلُ ثلاثةُ أجزاء لا يقوم بعضُها مقام بعض، **فيُحرَس كلٌّ على حدة**: الميتا تمنع
// التكبير، و`touch-action` يُسقط تكبيرَ النقر المزدوج ومهلةَ انتظاره معاً، وإعادةُ
// إعلان الميتا عند العودة تُلزم WebKit بها حيث يتلكّأ.
//
// **وصفحاتُ التعريف خارج القفل عمداً**: صفحاتُ كبارٍ والتكبيرُ فيها حقٌّ لا عيب —
// **وتُجرَد من القرص** كالألواح أعلاه، فصفحةٌ خامسة تُكتب غداً تدخل الحراسة يومَ تُكتب.
//
// **ولوحُ القلم محروسٌ من الجهة الأخرى**: `touch-action: none` عليه أخصُّ فيغلب، وسلوكُ
// اللمس تقاطعُ قيم العنصر وأسلافه (`none ∩ manipulation = none`) — فلو ابتلع القفلُ
// العامّ إعلانَ اللوح لصار الإصبعُ يمرّر الصفحةَ بدل أن يكتب. والدورةُ الكاملة بعد
// القفل مُثبَتةٌ في متصفّحٍ حقيقيّ (`browser_test.py --suite pen`).
...
ok(/html\s*\{[^}]*touch-action:\s*manipulation/.test(css),
  'ولا تكبيرَ بالنقر المزدوج في التطبيق كلِّه (touch-action على html) — والنقرةُ بلا مهلة انتظاره');
