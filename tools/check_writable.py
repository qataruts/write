...
#: **الحركاتُ تُؤجَّل** — حكمُ المالك (١٩ أغسطس ٢٠٢٦): «لا تُعرَض ولا تُطلَب»،
#: و«نؤجّل الشدّةَ والتنوين». **وقاعدةٌ تولّد الحكم**: ما زال بتجريد النصّ حركةٌ،
#: وما بقي حرفٌ — فالهمزةُ والتاءُ المربوطةُ حرفان يُدرَّبان وتحرسهما بطاقتاهما.
HARAKAT = "\u064b\u064c\u064d\u064e\u064f\u0650\u0651\u0652\u0670"


def marks_of(text: str, data: dict) -> list:
    """علاماتُ نصّ: كلُّ محرفٍ ليس من الحروف الثمانية والعشرين — **دون الحركات**.

    **ولا تُطلَب بطاقةٌ لما لا يُعرَض ولا يُكتب**: مادّتُنا مشكولةٌ في مصدرها
    (وهو صوابٌ يبقى للصوت والقارئ الآليّ)، **والمعروضُ والمطلوبُ مجرَّدان** —
    فمطالبةُ الحركة ببطاقةٍ مطالبةٌ بتدريس ما أُجّل (قِيست ٥٩١٨ مخالفةً كاذبة).
    """
    return [ch for ch in text
            if ch != " " and ch not in data["LETTERS"] and ch not in HARAKAT]
...
def self_test() -> int:
    ...
    # **والمزروعُ يُحسب لا يُسمّى** (١٩ أغسطس ٢٠٢٦): كان يزرع في «sign-shadda»
    # بالاسم، **فسقط الحارسُ يومَ أُجّلت الشدّة** بحكم المالك — والعقدةُ تُطلب بما
    # يميّزها (أنّها تفتح بطاقةً) لا باسمها، فيبقى الحارسُ حيّاً وإن تبدّل المنهج.
    def plant_markless(clone):
        first_node(clone, lambda st, n: bool(n.get("marks")))[1].pop("marks")
    ...
    # **وآخرُ بطاقةٍ في الرحلة** — تُحسب من المادّة ولا تُسمّى، فتصلح لأيّ منهج.
    late = data["MARKS"][-1]

    def plant_mark_before_card(clone):
        # **كلمةُ آخرِ بطاقةٍ تُقدَّم** إلى محطة كلمات المجموعة الأولى وبطاقتُها
        # باقيةٌ في موضعها المتأخّر — فتُطلب كتابةُ علامةٍ لم يعرفها الطفلُ بعد.
        node = node_of(clone, "g1-words", "w")
        node["words"] = [*node["words"], late["word"]]
    bad = with_(plant_mark_before_card)
    ok(any("بلا بطاقةِ تعريفٍ قبلها" in line and late["mark"] in line for line in bad),
