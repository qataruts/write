...
SISTERS = {"ش": "س", "ض": "ص", "ة": "ه"}
# **و«ة» أختُ «ه» بجسمٍ واحد** (طبقةُ العلامات، ١٩ أغسطس ٢٠٢٦): جسمُها جسمُ الهاء
# حرفاً بحرف ونقطتاها فوقه — **فتُقاس بجسمها** كما تُقاس «ض» بجسم «ص»، وإلا حُسبت
# فرجةُ النقطتين ارتفاعاً في الحرف فطال عن أطول الهجاء (قِيس ١٫٧٨١ والحدُّ ١٫٦١٦).
...
# ————— **والحدُّ العامل لا الثابتُ المكتوب** (عهدُ ١٩ أغسطس ٢٠٢٦) —————
#
# **علّةُ حكم المالك مقيسةٌ في المحرّك نفسِه** (`STROKE_ORDER §٩ب`): لوحُنا فيه
# **دائرةُ قبولٍ للنقطة**، فإن جاوز نصفُ قطرها الفرجةَ **دخلت الدائرةُ جسمَ الحرف
# فقُبلت نقطةٌ يضعها الطفلُ على الحلقة**. ⇐ **فالمقيسُ به دائرةُ القبول لا رقمٌ عامّ.**
#
# **و`pen.resolveTolerance` يضرب نصفَ قطر قبول النقطة في مقياس المادّة** — فقبولُ
# `ض/معزول` **٢٣٠ لا ١٤٠**، **وحدُّ ١٩٦ دونه**: اثنتا عشرةَ نقطةً من ثمانٍ وثمانين
# كانت فرجتُها دون قبولها العامل وهي فوق الحدّ المكتوب. **فيُطلَب الحدُّ من الدالّة
# التي تحكم به** (`scaled` أدناه — نظيرةُ `resolveTolerance` بحرفها) **لا من ثابت**.
#
# **وحدُّ المالك يبقى أرضيّةً**: الحدُّ **أكبرُ** الاثنين — فما صغُر مقياسُه لم يُرَدّ
# إلى ما دون «مسافةٍ واضحة»، وما كبُر أُخذ بقبوله هو.
#
# **والهامشُ معلَنٌ ومقيس، ولا يُخترَع له رقم**: هو **هامشُ التقريب** في `lift_dots`
# أدناه (`round(lift + 0.05, 1)`) — عُشرُ وحدةٍ فوق الحدّ، **فلا تُماسّ دائرةُ
# القبولِ الحبرَ**. ⚠ **ولم يُزَد عليه**: كلُّ زيادةٍ تُنفَق من هامش المطابقة
# (`MARGIN_OF_FIT`) الذي يحرس **أمرَ النسبة المنتهي** — انظر `fit_room` أدناه.


def dot_clearance(tol: dict, scale: float, unit: float) -> float:
    """**حدُّ فرجة النقطة لهذا الشكل**: أكبرُ أرضيّةِ المالك ونصفِ قطر قبوله العامل.

    `tol` سماحاتُ المحرّك كما تُقرأ من `pen.js`، و`scale` مقياسُ المادّة الذي
    يسافر معها (`ref.tolerance`) — **وضربُهما هو ما يفعله `resolveTolerance`**.
    """
    return max(DOT_CLEARANCE * unit, scaled(tol, scale)["dot"])
...
def fit_room(shape: dict, row: dict, unit: float, kin: str) -> float:
    """**كم يحتمل هذا الشكلُ من رفعٍ قبل أن يكسر أمرَ النسبة؟** — بالوحدات.

    **أمرُ النسبة منتهٍ لا جدال فيه** (أمرُ المالك ١٩ أغسطس ٢٠٢٦): ارتفاعُ الشكل
    عينُ ما يقوله المرجعُ بهامش `MARGIN_OF_FIT`، **ورفعُ النقطة يزيد الارتفاع**
    حيث تُعَدّ النقطةُ من حبر الشكل. ⇐ **فالرفعُ يقف عند حدّ النسبة ولا يتخطّاه**،
    وما لم يبلغ حدَّه يُبلَّغ برقمه ولا يُدَسّ.

    **ومن قِيست نسبتُه بجسمه وحدَه** (الأخوات: `ش` و`ض`) **فلا سقفَ عليه** — نقطُه
    خارجُ المقيس أصلاً. **ومن لا سندَ له في المرجع** فلا نسبةَ تُحرَس فيه.
    """
    if kin or not row:
        return math.inf
    _, _, y0, y1 = ink(shape)
    return (row["up"] + row["down"] + MARGIN_OF_FIT) * unit - (y1 - y0)


def lift_dots(shape: dict, want: float, room: float = math.inf) -> float:
    ...
    وقيودُه من نصّ الحكم:
      ...
      · **ولا تُرفَع نقطةٌ ليست فوق جسمها**: حكمُ المالك في «الحرف ونقطته» فوقه،
        ...
        شكلٍ لم يصدر.
      · **ولا يكسر الرفعُ أمرَ النسبة** (`room` من `fit_room`): أمرٌ منتهٍ أعلى من
        حدِّ الفرجة — **فمن لم يسعه سقفُه لم يُرفَع أصلاً ويُبلَّغ**، ولا يُرفَع
        نصفَ رفعةٍ فيُنفَق هامشُ النسبة كلُّه ولا يُبلَغ حدُّ الفرجة.
    """
    ...
    **والأفقُ**: المعزولُ يتوسّط خليّتَه (أمرُ المالك)، والموصولُ يبقى على إزاحته
    ...
    for ch, forms in paths.items():
        ...
        for form in FORMS:
            ...
            need = dot_clearance(tol, scale, unit)
            room = fit_room(out[ch][form], row, unit, kin)
            up_dots = lift_dots(out[ch][form], need, room)
            ...
            report.append({
                ...
                # **وحدُّ فرجته العامل ومتّسعُ نسبته** — فما لم يبلغ حدَّه يُعرَف
                # بكم قصّر ولماذا، **ولا يمرّ نقصٌ صامتاً**.
                "dot_want": round(need, 1),
                "fit_room": (None if room == math.inf else round(room, 1)),
