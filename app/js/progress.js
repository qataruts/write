...
export function journey() {
  ...
  for (const stage of STAGES) {
    // 🔴 **مرسوم ٢٤ أغسطس ٢٠٢٦ — «ابدأ من الحروف مباشرة دون هذه التهيئة،
    // الخطوطُ والمنحنياتُ غيرُ مفيدة»**: مرحلةُ التهيئة تُطوى من الرحلة
    // بياناً — عقدُها لا تدخل الخريطةَ ولا القفل، والحروفُ بابُ البداية.
    // (شاشتُها ومسارُها باقيان لمن قصدهما بيده — طيٌّ لا هدم.)
...
/**
 * **رحلةٌ بكرٌ لم تُمَسّ بعد**: صفرُ نجومٍ **وصفرُ محاولاتٍ في ليتنر**.
 *
 * تقرؤها بطاقةُ أوّل تشغيل (`firstrun.js`) — وهي وحدَها شرطُ ظهورها، فطفلٌ يمشي منذ
 * أسبوع لا يراها أبداً. **والشرطان معاً لا أحدُهما**: النجومُ وحدَها تكذب على مَن
 * جرّب محطةً ولم يُتمّها (كتب فأخطأ فخرج) — وقد مسّ الرحلةَ فعلاً، وبطاقةُ «ابدأ
 * من أوّلها» بعد ذلك خطابٌ في غير موضعه.
 *
 * **ومحوُ التقدّم يردّها بكراً** (`reset` يُفرِغ الحالة كلَّها) — وهو الصواب: مَن
 * محا ليبدأ من جديد فرحلتُه بكرٌ حقاً.
 */
export const untouched = () =>
  totalStars() === 0 && Object.keys(state.skills).length === 0;
