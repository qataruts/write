...
export function penSurface(config) {
  ...
  return {
    ...
    /**
     * **حبرُ المحاولة نقاطاً** — نسخةٌ تُسلَّم لمن يقرأ الكلمة (`reader.js`).
     *
     * 🔒 **واللوحُ لا يعرف الشبكة بحال** (عهدُ العائلة): يعطي نسخةً ولا يرسل، ومَن
     * أخذها هو المسؤول عن إذنِه — فيبقى `pen.js` نظيفَ الشبكة كما يحرسه فاحصُه.
     * **وصندوقُ المادّة معه** لأنّ القارئَ يحتاج مقاسَ ساحة الكتابة لا غير.
     */
    raw() {
      return attemptInk.map((st) => st.map((q) => [q[0], q[1], q[2] ?? null]));
    },
    area() {
      return { w: bw, h: bh };
    },
