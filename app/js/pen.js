...
export function partsOf(ref) {
  ...
  for (const stroke of ref?.strokes || []) {
    ...
    parts.push({ kind: 'stroke', poly, start: stroke.start || stroke.points[0], end: poly.pts[poly.pts.length - 1] });
  }
...
export function createTrial(ref, options = {}) {
  ...
  function move(x, y) {
    ...
    const hit = nearestOn(poly, p, stroke.cursor - tol.back * 2, stroke.cursor + ahead);
