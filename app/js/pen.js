 ...
 * واحدة**. ويكفُّ ضررَها اليومَ أنّ `check_paths.py` يحدّ طولَ القطعة، ومَن احتاج
 * حدّاً قاطعاً حبَسه عنده (انظر `within` في الطيّة). **وإحكامُها في `nearestOn` نفسِها
 * يشدّ المحرّكَ على القِسِيّ الضيّقة** (تُوَيْجُ ب/نهائي: هامشُ رجفته يهبط من ٥١ إلى
 * ٣٠) — فهو تبديلٌ في الحكم يستأهل جلستَه وبلاغُه عند المدير.
 */
...
export function createTrial(ref, options = {}) {
  ...
  function move(x, y) {
    ...
    //
    // **فالإعلانُ يرفع اللبس بقسمة الطيّة على القمّة**: ما لم تُبلَغ القمّةُ فالمقيسُ
    // **الضلعُ الصاعد وحدَه**، وبعدها **النازلُ وحدَه**. فيبقى التقدّمُ رتيباً في
    ...
    const climb = () => within(nearestOn(poly, p, lo, Math.max(lo, Math.min(hi, fold.apex))),
      lo, fold.apex);
    const fall = () => within(nearestOn(poly, p, Math.min(hi, Math.max(lo, fold.apex)), hi),
      fold.apex, hi);
    let hit;
    if (!fold) {
