...
export const STEPS = [
  ...
  /* 🔴 **والكلماتُ حرّةٌ بلا مسارٍ يُعرَض** (حكم المالك ٢٥ أغسطس ٢٠٢٦: «أرى في
     الكلمات المحرّكَ يحاول أن يمشي على الحروف لكنه لا يمشي مع الكتابة بالشكل
     الصحيح وهذا يشتّت — أقترح أن نلغي إظهارَ المسار على الكلمات وتركَها حرّة…
     وإبقاءَ حركة مسار الكتابة في الحروف فقط»). **وعلّتُه مقيسةٌ ومكتوبة**: مادّةُ
     النسخ تعود على أثرها كثيراً (القطعُ إلى أدناه بأمره) فيقفز مؤشّرُ التقدّم بين
     ضلعي الطيّة — فيرى الطفلُ قلماً يتردّد. ⇐ **الدرجتان حرّتان**: النموذجُ محجوب
     والكلمةُ المكتوبةُ فوق اللوح هي المرجع، **والحرفُ على حاله** (تتبّعٌ وعرضٌ
     متحرك) لأنّ مساره لا يعود على أثره إلا في مواضعَ معدودةٍ محروسة. */
  { id: 'guided', mode: MODES.FREE, title: 'اِنْسَخْ', say: SAY.guided, kind: progress.KINDS.COPY },
  ...
  function paintFoot(step, unit) {
    ...
    const measureOn = async () => {
      ...
      /* 🔴 **وقياسُ الطريقة مرفوعٌ عن الكلمات** (حكم المالك ٢٥ أغسطس ٢٠٢٦، بعد شحن
         ...
         الطريقة… في الكلمات أقصد نلغيه»). **وعلّتُه مقيسة**: مادّةُ النسخ الجديدة
         تعود على أثرها كثيراً (القطعُ إلى أدناه بأمره) — والماشي يقرأ الرجوعَ تحت
         رجفة الطفل انحرافاً (دَينُ «قراءةُ الطيّة تحت الرجفة»). ⇐ **يبقى قياسُ
         الشكل** (تغطيةً ودقّةً — وهو مصدرُ النجوم ولوحة الأهل) **ويسقط وصفُ
