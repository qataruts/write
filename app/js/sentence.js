...
//
// **وحلقتُها خطوتان**: **انْسُخْ** (النموذجُ كاملاً على مسطرته) ثم **اُكْتُبْ سَمَاعاً**
...
// `fade.js`): الجملةُ تُنسَخ فتُملى في مجلسها، لأن كلماتِها كلَّها **دُرِّست كتابةً
// وأُمليت** قبلها، والمستحدَثُ هنا السطرُ نفسُه لا حروفُه.
//
 ...
 */
export const SAY = {
  copy: 'اُنْسُخِ الْجُمْلَةَ عَلَى السَّطْرْ',
  dictate: 'اِسْتَمِعْ ثُمَّ اُكْتُبِ الْجُمْلَةْ',
};
...
export const STEPS = [
  { id: 'copy', mode: MODES.GUIDED, title: 'اُنْسُخْ', say: SAY.copy, kind: progress.KINDS.COPY },
...
export const nodeOf = (part) => progress.allNodes()
  .find((node) => node.type === 'sentence' && node.part === part) || null;
...
export const renderSentence = (part) => renderNode(nodeOf(part));
